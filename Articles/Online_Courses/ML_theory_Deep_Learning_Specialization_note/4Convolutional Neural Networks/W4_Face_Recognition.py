...
def triplet_loss(y_true, y_pred, alpha = 0.2):
    ...
    ### START CODE HERE
    #(≈ 4 lines)
    #print(tf.shape(tf.square(tf.subtract(anchor,positive))))
...
def verify(image_path, identity, database, model):
    ...
    x1=database[identity]
    x2=encoding
    #dist = np.sqrt(tf.reduce_sum(tf.square(tf.subtract(x1,x2)),axis=-1))
    [dist] = np.linalg.norm(tf.subtract(x1,x2),axis=-1)
...
def who_is_it(image_path, database, model):
    ...
    for (name, db_enc) in database.items():
        ...
        [dist] = np.linalg.norm(tf.subtract(db_enc,encoding),axis=-1)
