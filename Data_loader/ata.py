...
class IterableJoblibDataset_dict(IterableDataset):
    def __init__(self, file_list, input_keys, output_keys, batch_size):
    ...
    def setup(self):
        ...
        for data in self.joblib_files:
            file_size = len(data[input_keys[0]])
    ...
    def __iter__(self):
        ...
        for file_idx, data in file_iter:
            input_data = {data[i] for i in input_keys}
            target_data = {data[i] for i in output_keys}
            
            for i in range(0, len(input_data[input_keys[0]]), self.batch_size):
                batch_input = {key: torch.from_numpy(input_data[key][i:i+self.batch_size]).float().to(self.device) for key in input_keys}
                batch_target = {key: torch.from_numpy(target_data[key][i:i+self.batch_size]).float().to(self.device) for key in input_keys}
                yield batch_input, batch_target

def iter_loader_dict(file_list, batch_size=1000):
    ...
    for input_data, target_data in data_loader:
        ...
        output_dims={key:target_data[key].shape[1] for key in target_data}

        seq_len=input_data[list(input_data.keys())[0]].shape[1]

        print(f"Sample input shape: {input_data[input_data.keys()[0]].shape}")
        print(f"Sample target shape: {target_data[input_data.keys()[0]].shape}")
        break
    
    return dataset, data_loader, input_dim, output_dim, seq_len
