...
def forward_propagation_with_dropout(X, parameters, keep_prob = 0.5):
    ...
    # YOUR CODE STARTS HERE
    (c_temp,r_temp)=np.shape(A1)
    D1 =np.random.rand(c_temp,r_temp)                                           # Step 1: initialize matrix D1 = np.random.rand(..., ...)
    #print(D1)
    ...
    # YOUR CODE STARTS HERE
    #print(np.shape(A2))
    (c_temp,r_temp)=np.shape(A2)
    D2 =np.random.rand(c_temp,r_temp)  
    #print(np.shape(D2))
    #D2 =np.random.rand(c_temp)                                           # Step 1: initialize matrix D1 = np.random.rand(..., ...)
...
def backward_propagation_with_dropout(X, Y, cache, keep_prob):
    ...
    dA2 = dA2*D2               # Step 1: Apply mask D1 to shut down the same neurons as during the forward propagation
