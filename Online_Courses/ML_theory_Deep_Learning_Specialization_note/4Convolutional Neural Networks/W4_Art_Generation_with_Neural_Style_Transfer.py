...
def compute_layer_style_cost(a_S, a_G):
    ...
    a_S = tf.reshape(np.transpose(a_S), shape=[n_C, n_H*n_W])
    a_G = tf.reshape(np.transpose(a_G), shape=[n_C, n_H*n_W])
...
def train_step(generated_image):
    with tf.GradientTape() as tape:
        ...
        #(1 line)
        # J_style = compute_style_cost(style_image_output, generated_image_output, STYLE_LAYERS=STYLE_LAYERS)
        print('****************')
