...
def compute_cost(logits, labels):
    ...
    #temp=tf.nn.softmax_cross_entropy_with_logits(labels=labels,logits=logits)
    temp=tf.keras.losses.categorical_crossentropy(labels,logits,from_logits=True)
    cost = tf.math.reduce_mean(temp)
    #cost = np.sum(temp)/2
    print(cost)
    # YOUR CODE ENDS HERE
    return cost
