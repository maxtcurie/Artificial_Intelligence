...
def select_threshold(y_val, p_val): 
    ...
    for epsilon in np.arange(min(p_val), max(p_val), step_size):
        ...
        for i in range(len(p_val)):
            if   p_val[i]<epsilon and y_val[i]:
                tp=tp+1.
            elif p_val[i]<epsilon and not y_val[i]:
                fp=fp+1.
            elif p_val[i]>=epsilon and y_val[i]:
                fn=fn+1.
        '''
        
        
        filter_arr = (p_val<epsilon)
        a=[0       if (filter_arr[i])     and (y_val[i])             else 1 if (filter_arr[i]) and ( not y_val[i])             else 2 if ( not filter_arr[i]) and ( y_val[i])             else 3             for i in range(len(p_val))]
        a=np.array(a,dtype=int)
        
        tp=float(np.count_nonzero((a==0)))
        fp=float(np.count_nonzero((a==1)))
        fn=float(np.count_nonzero((a==2)))
